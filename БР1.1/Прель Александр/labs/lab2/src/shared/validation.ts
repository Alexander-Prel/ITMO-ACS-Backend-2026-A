...
export const asPositiveInt = (value: unknown): number | undefined => {
  ...
  return Number.isSafeInteger(raw) && Number(raw) > 0 ? Number(raw) : undefined;
};

export const isEmail = (value: string) => emailRegex.test(value);
export const isPhone = (value: string) => phoneRegex.test(value);
export const isDate = (value: string) => dateRegex.test(value) && !Number.isNaN(Date.parse(value)) && new Date(value).toISOString().slice(0, 10) === value;
